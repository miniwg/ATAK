...
##Mark Halliday Mini workgroups ltd - v002.01 - 10 May 2025
...
##Set path for certs-metadata.sh file
TAK_META=$TAK_LOCATION/certs/certs-metadata.sh
...
(*)
   ...
   sudo openssl pkcs12 -export -in /etc/letsencrypt/live/$certNameVar/fullchain.pem -inkey /etc/letsencrypt/live/$certNameVar/privkey.pem -out takserver-le.p12 -name $certNameVar -password pass:$TAKPASS
   ...
   sudo keytool -importkeystore -srcstorepass $TAKPASS -deststorepass $TAKPASS -destkeystore takserver-le.jks -srckeystore takserver-le.p12 -srcstoretype pkcs12
